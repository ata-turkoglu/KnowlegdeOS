...
> = {
  ...
  'locked-rules': { tr: 'Kural Tabanlı Filtre Çıkarımı' },
...
const chatTechnicalTitles: Record<string, string> = {
  ...
  'locked-rules': 'Rule-Based Filter Extraction',
...
const chatEdges = workflowEdges([
  ...
  ['normalize', 'locked-rules'],
  ['normalize', 'analysis'],
  ['locked-rules', 'analysis'],
  ['analysis', 'planner'],
...
const uploadNodes: WorkflowNode[] = [
  ...
  node(
    ...
    'Indexing preference',
    'decision',
    'Batch reindex isteğindeki useLlm seçimi, model ile ek metadata/entity extraction uygulanıp uygulanmayacağını belirler.',
  ),
  node(
    'llm-extract',
    2860,
    650,
    'Metadata ve entity bağlama',
    'Zengin metadata çıkarımı',
    'local-llm',
    'Etkinse seçili model, doküman bağlamından ek yapılandırılmış metadata/varlık üretir; çıktı iş kurallarıyla doğrulanır.',
    undefined,
    'entityLinkerModel',
  ),
  node(
...
const uploadEdges = workflowEdges([
  ...
  ['extract', 'llm-extract-decision'],
  ['llm-extract-decision', 'llm-extract', 'Evet'],
  ['llm-extract-decision', 'field-match', 'Hayır'],
  ['llm-extract', 'field-match'],
  ['llm-extract', 'write-claims', 'Verified INSERT'],
  ['field-match', 'alias'],
