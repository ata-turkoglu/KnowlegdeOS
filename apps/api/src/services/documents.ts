...
import { initialIndexingStageResults, resolveIndexingPlan, type IndexingPlan, type IndexingProviderPreferences, type IndexingRequestMode, type IndexingStageName } from './indexing-plan.js';
...
export async function reindexStoredDocument(
  config: ApiConfig,
  input: { workspaceSlug: string; documentName: string; mode?: IndexingRequestMode; requestedStages?: Partial<Record<IndexingStageName, boolean>>; providerPreferences?: IndexingProviderPreferences; plan?: IndexingPlan; signal?: AbortSignal; invalidateSemanticIndex?: boolean; onProgress?: (stage: string) => void }
  ...
  const result = await withDb(config, async ({ db }) => {
    ...
    const plan = input.plan ?? resolveIndexingPlan(config, { mode: input.mode, workspaceId: ws.id, documentId: document.id, hasFrontmatter: Object.keys(ingestion.frontmatter).length > 0, chunkCount: ingestion.chunks.length, requestedStages: input.requestedStages, providerPreferences: input.providerPreferences });
...
export async function reindexWorkspaceDocuments(config: ApiConfig, workspaceSlug: string, options: { signal?: AbortSignal; mode?: IndexingRequestMode; requestedStages?: Partial<Record<IndexingStageName, boolean>>; providerPreferences?: IndexingProviderPreferences; onProgress?: (progress: { completed: number; total: number; documentName?: string }) => void } = {}) { const items = await listStoredDocuments(config, workspaceSlug); let completed = 0; for (const item of items) { if (options.signal?.aborted) throw new Error("Reindexing cancelled."); options.onProgress?.({ completed, total: items.length, documentName: item.documentName }); await reindexStoredDocument(config, { workspaceSlug, documentName: item.documentName, mode: options.mode, requestedStages: options.requestedStages, providerPreferences: options.providerPreferences, signal: options.signal }); completed++; } return { workspaceSlug: slugify(workspaceSlug), reindexedCount: completed }; }
