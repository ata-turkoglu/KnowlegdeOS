...
For a controlled workspace, first regenerate YAML from the Convert screen (or its YAML batch action), inspect the resulting trace files when `METADATA_DIAGNOSTICS_ENABLED=true`, then start that workspace's reindex operation in automatic mode. Do not run a production-wide regeneration until the audit report and a controlled reindex have been reviewed. The reindex endpoint is `POST /api/settings/ingestion/:workspaceSlug/reindex`; omit the deprecated `useLlm` field so the persisted stage plan can route deterministic and LLM-dependent stages independently. Set `INDEXING_DIAGNOSTICS_ENABLED=true` to retain bounded parsed stage traces under workspace metadata. Trace payloads are capped at 256 KB, redact token-shaped values, and expire after `INDEXING_DIAGNOSTICS_RETENTION_DAYS` (default 7). Set `RAW_MODEL_OUTPUT_DIAGNOSTICS_ENABLED=true` only for a controlled investigation to retain redacted model output; it is disabled by default, bounded by `RAW_MODEL_OUTPUT_MAX_CHARACTERS` (default 32,000), shares the diagnostic retention window, and never stores prompts, HTTP transport bodies, credentials, or headers. Roll back a YAML regeneration by restoring the prior Markdown files from backup, then reindex the restored workspace.

For a controlled CLI rebuild, first run `corepack pnpm metadata:audit --root=converted-markdown`, then preview the stored-document scope with `corepack pnpm indexing:rebuild --workspace=<slug> --regenerate-yaml --dry-run`. The YAML option regenerates and serialization-validates each selected stored Markdown document before reindexing. It makes no writes unless both `--apply --confirm-rebuild` are supplied, for example: `corepack pnpm indexing:rebuild --workspace=<slug> --regenerate-yaml --batch-size=10 --apply --confirm-rebuild --output=rebuild-report.json`. Omit `--regenerate-yaml` only when the stored YAML has already been reviewed and should be preserved; the command otherwise rebuilds derived index data and embeddings.
...
Audit persisted indexing history before a broader rebuild with `corepack pnpm indexing:audit --format=text`; use the default JSON output or `--output=graph-audit.json` for an archival report. It flags legacy global-LLM history, missing per-stage records, failed stages reported as completed, and zero-accepted or fully rejected alias/relationship candidates. Add `--database` for a read-only database check of stale LLM graph rows and unreferenced entities. When diagnostics are enabled, each document's operation row links to its retained trace; expired or disabled traces return 404 rather than falling back to an unbounded log.

Graph stages have independent configured routes: `INDEXING_ALIAS_MODEL`, `INDEXING_RELATIONSHIP_MODEL`, `INDEXING_CLAIM_MODEL`, and `INDEXING_SUMMARY_MODEL`. Each accepts either an Ollama model (`qwen3:8b` or `ollama/qwen3:8b`) or an explicit API route (`openai/model`, `gemini/model`, `anthropic/model`). A user-configured reindex request may override these per stage with `providerPreferences`; the resolved choice is persisted in the indexing plan.
